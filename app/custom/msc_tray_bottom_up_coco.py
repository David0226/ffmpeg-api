...
class MSCTrayBottomUp(Kpt2dSviewRgbImgBottomUpDataset):
...
#             cfg = Config.fromfile('configs/_base_/datasets/coco_cube.py')
            cfg = Config.fromfile('configs/_base_/datasets/coco_msc_tray.py')
    ...
    def evaluate(self, results, res_folder=None, metric='mAP', **kwargs):
        ...
        for idx, _preds in enumerate(preds):
            ...
            ### Correct Error ###
            # annotation file에서 images의 file_name에 폴더명이 포함되어 있을 경우 오류 발생
            # osp.basename() 함수가 파일명만 추출하여 list에서 이름으로 찾지 못함
#             image_id = self.name2id[osp.basename(str_image_path)]
            image_id = self.name2id[osp.relpath(str_image_path, self.img_prefix)]
